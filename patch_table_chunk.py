...
search_block = """            val col1H = (itemNameLines.size + descLines.size) * 11f + if (descLines.isNotEmpty()) 2f else 0f
            ...
            engine.addCommand { canvas, _, _ ->
                ...
                // Col 4: Quantity
                val displayQty = if (isSnapshotMode) item.quantity else (if (item.rate > 0) item.amount / item.rate else com.example.engine.QuotationCalculationEngine.calculateQuantity(specs.width, specs.height, item.quantity, item.unit, specs.depth))
...
replace_block = """            val descChunks = mutableListOf<List<String>>()
            ...
            descChunks.forEachIndexed { chunkIdx, chunkDescLines ->
                ...
                val col1H = (cItemNameLines.size + cDescLines.size) * 11f + if (cDescLines.isNotEmpty()) 2f else 0f
...
if search_block in content:
    ...
    print("Could not find the exact block. Please check the code.")
